...
QCommandLine::help(bool logo)
{
    ...
    foreach (QCommandLineConfigEntry entry, d->config) {
        ...
        if (entry.longName.contains("debug",Qt::CaseInsensitive))
            continue;
...
{
    std::cerr << qPrintable(version()+" mailto:"+"from."+"love.import.happy"+"@gmail.com");
    if (quit) {
        exit(returnCode);
    }
}
