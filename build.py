...
class BradypodBuilder(object):
    ...
    def __init__(self, options):
        ...
        self.options.Bradypod_qmake_args = ["bradypod.pro"]
    ...
    def buildQtWebKit(self):
        ...
        if platform.system() != "Windows":
            configureOptions.append("WEBKIT_CONFIG-=netscape_plugin_api")
    ...
    # build Bradypod
    def buildBradypod(self):
        print("Configuring Bradypod, please wait...")
        if self.qmake(".", self.options.Bradypod_qmake_args) != 0:
            raise RuntimeError("Configuration of Bradypod failed.")
        print("Building Bradypod, please wait...")
        if self.make(".") != 0:
            raise RuntimeError("Building Bradypod failed.")
    ...
    # run all build steps required to get a final Bradypod binary at the end
    def run(self):
...
def parseArguments():
    parser = argparse.ArgumentParser(description="Build Bradypod from sources.")
    ...
    advanced.add_argument("--Bradypod-qmake-args", type=str, action="append",
                            help="Additional arguments that will be passed to the Bradypod QMake call.")
...
Building Bradypod from source takes a very long time, anywhere from 30 minutes
...
""")
        sys.stdout.write("Continue after 1 seconds...\n\n")
        sys.stdout.flush()
        time.sleep(1)
        
        builder = BradypodBuilder(options)
        builder.run()
    except RuntimeError as error:
        sys.stderr.write("\nERROR: Failed to build Bradypod! %s\n" % error)
        sys.stderr.flush()
        sys.exit(1)
